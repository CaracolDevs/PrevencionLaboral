...
const value = require('../models/value').dangerProcessValue;
...
let riesgoConsecuencia = ''
    ...
    if(body.controlesOperacionales) {
    ...
    */
    ...
    // procesar reevaluacion

    let reconsecuencias = ''

    for (let value of concecuenciaResultValues) {
        if(body.reconsecuenciasResult  == value.id) {
            reconsecuencias = value.name
        }
    }
    ...
    for (let value of riesgoResultValues) {
        if(body.reriesgoResult  == value.id) {
            reriesgo = value.name
        }
    }
    ...
    for (let value of exposicionResultValues) {
        if(body.reexposicionResult  == value.id) {
            reexposicion = value.name
        }
    }

    //procesar remake del result

    //procesar result

    let reresult = body.reconsecuenciasResult * body.reexposicionResult * body.reriesgoResult
    let reresultText = ''
    ...
    if(reresult >= 20 && reresult < 70) {
        reresultText = 'Debe corregirse pero no es una emergencia.'
    }

    if(reresult >= 70 && reresult < 200) {
        reresultText = 'Riesgo notable'
    }

    if(reresult >= 200 && reresult < 400) {
        reresultText = 'Correción inmediata a través de un plan de acción.'
    }

    if( reresult > 400) {
        reresultText = 'Detención inmediata de la actividad peligrosa y elaboración de un plan de acción.'
    }
   ...
   const addValue = await value.create({ 
        ...
        CIngenieria: body.cIngenieria,
        CAdministrativos: body.cAdministrativos,
        CPPersonal: body.cPPersonal,
        ...
        RemakeConsecuencias: consecuencias,
        RemakeConcecuenciasNumber: body.consecuenciasResult,
        RemakeExposicion: exposicion,
        RemakeExposicionNumber: body.exposicionResult,
        RemakeRiesgo: riesgo,
        RemakeRiesgoNumber: body.riesgoResult,
        RemakeResult: reresult,
        RemakeResultText: reresultText
...
exports.delete = async (req, res) => {
    let resultToDelete = await value.findAll({where: {
...
exports.edit = async (req, res) => {
    let result = await value.findAll({where: {
        ProcesoId: req.params.id
    } });
...
exports.editProcess = async (req, res) => {
    let actual = await value.findAll({where: {
        ProcesoId: req.params.id
    } });
    let body = req.body
